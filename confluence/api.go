...
func NewAPI(baseURL string, username string, password string, insecureSkipVerify bool) *API {
	...
	restV2 := gopencils.Api(baseURL+"/wiki/api/v2", auth, httpClient, 3) // v2 API for folders and new features
	if username == "" {
...
func (api *API) CreateFolder(spaceID, title string, parentID *string) (*FolderInfo, error) {
	...
	if err != nil {
		return nil, karma.Format(
			err,
			"failed to create folder %s in space %s",
			title,
			spaceID,
		)
	}
