...
// them. It sits below resty, whose own retry is left off, so this is the only
// place retries happen -- and unlike resty's default conditions it is
// method-aware, so a POST that failed with a 5xx is never replayed.
...
func (t *retryTransport) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	// populates GetBody for the in-memory bodies resty builds, and
	// replayableBody buffers the one it streams, a multipart upload.
