...
func (r *ConfluenceTextRenderer) renderText(writer util.BufWriter, source []byte, node ast.Node, entering bool) (ast.WalkStatus, error) {
	if !entering {
			...
			if r.EastAsianLineBreaks != html.EastAsianLineBreaksNone && len(value) != 0 {
				...
				if sibling != nil && sibling.Kind() == ast.KindText {
					if siblingText := sibling.(*ast.Text).Value(source); len(siblingText) != 0 {
						...
						if writeLineBreak {
							_ = writer.WriteByte(byte(r.softBreak))
						}
					}
				}
			} else {
